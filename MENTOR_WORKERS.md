...
3. worker 103 is merged as `cafa32c`: token execution is implemented but `CF-FARP@1.0` remains draft/unsupported until the protocol owner promotes Base support. Worker 104 is rejected pending a narrow fix: its Analyzer projection must distinguish “plan compiled” from “Base runnable/publishable” while that gate remains. They have no shared write paths after that fix is accepted.
...
| worker-104-plan-projection | rejected | Project ExecutionPlan semantics into Analyzer and engineering view; visual executable edges must agree with the plan. | `src/core/lab/flow_analyzer.py`, `src/frontend/src/pages/flow-workbench/**`, directly related analyzer/UI tests | `src/core/orchestration/**`, `src/core/protocol/**`, `protocol/**`, `src/core/cartridge/**`, `src/core/runtime/**`, `src/backend/**`, `MENTOR_WORKERS.md` | worker 102 merged as `2adc446`; repair current rejected commit | `workers/worker-104-plan-projection` | `..\CartridgeFlow-worker-104-plan-projection` | Rejected: compiler success is incorrectly marked runnable/packagable/publishable although `CF-FARP@1.0` is draft/unsupported. Fix must preserve plan-edge projection while reporting Base non-runnability until runtime support exists. |
...
- Mentor acceptance: Rejected on 2026-07-30. In `_analyze_v10_execution_plan`, a successfully compiled draft plan returns `runnable: true`, and allows `packagable`/`publishable` for package/publish targets. This contradicts `protocol/catalog/release_manifest.json`, which marks `CF-FARP@1.0` as `draft` and `unsupported`; it can incorrectly open downstream gates before a token runner or Base support exists. Repair the projection so it may expose `execution_plan.status=compiled` and its visual plan edges, but it must remain non-runnable, non-packagable and non-publishable with an explicit Chinese unsupported-runtime diagnostic until the support declaration and runtime evidence are merged. Add a regression test for that distinction. Do not modify any other worker-owned path.
