# CartridgeFlow v0.1.0 AI Developer Guide
...
1. 阅读根目录 `README.md`、`GOAL.md` 和 `TODO.md`。
2. 阅读 `BASE_IMPLEMENTATION.json`，确认基座真实声明的协议、profile、capability 和 tool pack。
3. 涉及流程语义时，完整阅读 `docs/protocol/CARTRIDGEFLOW_FLOW_AUTHORING_RUNTIME_PROTOCOL_v0.5.md`。
4. 涉及 DLC 时，阅读 `docs/architecture/MCP_DLC_MODULE_BOUNDARY.md` 和 `core/extensions/`。
...
```powershell
Get-FileHash -Algorithm SHA256 cartridges/dev/<id>/dlc/backend/entry.py
```
...
- `VERSION`、`BASE_IMPLEMENTATION.json`、前端 package version 和 Git tag 一致。
