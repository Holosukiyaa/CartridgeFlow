...
| Protocol consumption | `config/protocol/`, `config/protocol-viewer/`, `src/core/protocol/`, `scripts/` | `protocol-registry.sqlite`, `protocol-registry.lock.json`, `datasette.json`, `templates/`, `requirements.txt`, `view-protocols.bat`, `launch_protocol_viewer.py`, `artifact_store.py`, `release_catalog.py`, `governance_registry.py`, `compatibility.py`; unique source: [cartridgeflow-protocols](https://github.com/Holosukiyaa/cartridgeflow-protocols) |
