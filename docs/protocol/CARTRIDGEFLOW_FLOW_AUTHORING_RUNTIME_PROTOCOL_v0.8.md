# CartridgeFlow Flow Authoring Runtime Protocol v0.8
...
关系：本文完整替代 v0.7，是独立、自包含的 Flow 创作、静态分析与运行规范。实现或认证 v0.8 不需要读取历史 CF-FARP 正文。卡带领域语义由卡带自己的 schema 与 Portable DLC 提供，不属于本协议。
...
1. v0.8 是完整快照，不从 v0.7 或更早版本继承隐含语义。
2. 已按其他版本认证的卡带不得使用 v0.8 标签。
3. Base 只有在 `supported_protocols` 中声明 v0.8 时才能运行该卡带。
...
3. `runtime_contract` MUST 指向 `CF-FARP@0.8`。
...
9. v0.8 卡带 MUST 要求 `flow_analysis` profile，以及 `structured_io_contract`、`explicit_input_binding`、`typed_control_edges`、`executable_topology_filter`、`flow_analysis_report_v1` 和 `analysis_report_freshness_guard` 最低能力；缺失时不得以 v0.8 运行或认证。
...
4. v0.8 registry 中 `executable` MUST 为 `false`。脚本、WebAssembly、Worker 和其他可执行内容不得作为普通资产注册。
...
7. 节点 SHOULD 使用稳定资产引用；兼容导入器 MAY 读取旧相对路径，但必须在保存 v0.8 Flow 前迁移为 asset ID。
...
远程知识库、搜索索引、数据库查询服务和内容仓库只要通过网络或本机进程提供能力，就属于本节的 MCP、remote API 或其他声明工具。v0.8 不定义独立的全局 Data Source 绑定；需要随卡带携带的静态知识内容应作为 package asset，需要运行时查询的外部内容必须经过工具契约、权限、超时和审计。
...
Root Flow MAY 使用节点 `next`、结构化 `routes`、`action_routes`、`failure_route` 或顶层 `control_edges` 表达可执行关系。v0.7 的顶层 `edges` 是迁移输入，不是 v0.8 的规范持久化字段。Base 必须先完成类型校验，再规范化为同一原子控制图：
...
静态分析至少检查 start、缺失节点、不可达节点、无出口节点、冲突边、未受控循环、数据链来源、分支数据可用性、资源绑定和副作用路径。分析发现与运行事件必须使用稳定 node id。v0.8 Base 必须通过第 44 节 Analyzer 生成规范化拓扑；前端不得维护另一套决定运行语义的图推导规则。
...
| `input` / `optional_input` | v0.7 迁移输入；保存为 v0.8 前必须规范化到 inputs |
| `output` | v0.7 迁移输入；保存为 v0.8 前必须规范化到 outputs |
| `input_schema` | 输入结构约束 |
...
v0.8 作者源文件 MUST 使用第 42 节定义的结构化 `inputs` 与 `outputs`。字符串、逗号/换行分隔字符串和字符串数组只允许被迁移器读取；Analyzer 对尚未迁移的写法必须产生 `LEGACY_IO_CONTRACT` finding，production、package 与 publish 目标下是 blocker。
...
10. `kind=ui` 在 v0.8 中不是合法别名。旧节点必须迁移为 interaction，并明确组件、模式、输入、输出与动作路由。
...
13. `script_policy` 在 v0.8 MUST 为 `external_hashed_only`：禁止 inline script、inline module、`eval`、`new Function`、字符串定时器、动态生成代码、WebAssembly 和未列入 files 的动态 import。
...
下列能力在 v0.8 中禁止授予 frontend component：Pending Interaction 最终提交、任意 Store 读写、任意节点执行、任意路由跳转、模型或工具直调、任意网络代理、凭据读取、Base 文件系统、主前端状态、其他卡带数据、任意 HTML 注入和永久后台任务。
...
旧协议如果位于 Base 历史索引，必须报告 recognized_unsupported_protocol 和迁移目标；未知协议报告 unknown_protocol。不得用当前 v0.8 解释器静默运行旧版本。
...
Base MUST 只声明已实现并有证据的能力。没有声明某能力不等于协议删除该能力，而是该 Base 只能支持不要求该能力的 v0.8 卡带。

## 35. 从 v0.6 迁移

迁移到 v0.8 至少完成：

1. Runtime Contract、Root Flow 和 certification target 改为 v0.8；Base Contract 继续独立声明。
...
12. 重新计算所有 registry、descriptor 和 package hash，运行 v0.8 conformance 和认证，不沿用 v0.6 标签。
...
- [ ] Manifest 同时声明 Base Contract 与 CF-FARP v0.8。
...
- [ ] 所有 v0.8 节点使用结构化 inputs、outputs、binding 和 schema。
...
| v0.6 内容 | v0.8 位置 | 处置 |
|---|---|---|
| 协议定位、关键词、独立 Base Contract | 1-3、6.3、32 | 完整保留；版本升级为独立 v0.8 快照 |
...
| HTML 相对路径或内联内容 | 6.7-6.9、35 | 替代：保存 v0.8 前迁移为 asset/component 引用；主动内容不得作为普通资产 |
...
| v0.6 认证标签 | 3、35 | 不沿用；v0.8 必须重新认证 |

本矩阵是覆盖审计，不表示 v0.8 运行时可以直接解释 v0.6 卡带。迁移必须生成 v0.8 卡带 revision，完成主动内容审计、重新计算 hash 并重新认证。
...
未来 v0.8 文案修正不得改变规范语义。新增 required 字段、状态、生命周期、副作用、所有权或安全边界必须发布新的完整协议版本。
...
v0.8 节点 MUST 使用具名 `inputs` 与 `outputs`。每个名称只在当前节点内作为端口 identity，不得依赖显示名称或字段相似度建立绑定。
...
Flow Analyzer 是 v0.8 的规范组件。它读取静态声明，生成规范化拓扑、派生关系和 finding；它不是 Runner，不执行卡带业务代码。
...
v0.8 Base 的 dev_authoring 实现 SHOULD 提供三组结构化接口。接口名称可以不同，但行为语义必须可映射。
...
v0.7 卡带迁移到 v0.8 MUST 生成新 revision 或副本，并至少完成：

1. Runtime Contract、Root Flow、Portable DLC protocol 和 certification target 更新为 v0.8，重新计算所有受影响 hash。
...
10. 运行 v0.8 conformance，重新认证；不得沿用 `cf-farp-0-7-certified`。
...
| v0.7 内容 | v0.8 位置 | 处置 |
|---|---|---|
| 协议身份、Manifest、资产与 Base Contract | 1-10 | 完整保留并升级为独立 v0.8 快照 |
| `next`、routes 与顶层 `edges` | 10、43、48 | 加强：规范字段改为 typed `control_edges`；旧 `edges` 仅供迁移 |
| Process Node、kind、executor、effect | 11-12 | 完整保留 |
| 字符串 `input`、`optional_input`、`output` | 11、42、48 | 替代：v0.8 作者事实使用结构化 inputs/outputs/binding |
...
| v0.7 认证标签 | 3、33、48 | 不沿用；v0.8 必须重新认证 |

本矩阵是覆盖审计，不表示 v0.8 Runner 可以直接解释 v0.7 卡带。迁移必须生成 v0.8 Authoring Facts、全量分析报告、新 hash 和新认证证据。

## 50. 统一 Flow 资源目录

v0.8 Base MUST 为每个 Flow 生成唯一的只读资源解析结果 `cartridgeflow.flow_resource_catalog.v1`。工具管理、模型管理、Analyzer、运行预检、Runner snapshot 和打包预检 MUST 消费同一目录，不得分别从本机配置、Manifest 或节点字段自行拼接另一套事实。
