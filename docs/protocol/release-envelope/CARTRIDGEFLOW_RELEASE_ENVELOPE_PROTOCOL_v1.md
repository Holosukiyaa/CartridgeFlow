...
当前仓库已实现本协议的静态结构、公开合同泄露、路径和摘要验证器。这不表示基础宿主、开发台、个人运行台或市场已经完整支持签名验签、安装、升级、回滚或执行 `CF-CRE@1` 发行包。`config/base/BASE_IMPLEMENTATION.json` 不得仅因本协议存在就声明新的运行支持。
...
本协议现在定义了签名输入和元数据；在当前仅静态验证状态中，密码学签名验证尚未实现。因此，只有结构合法签名元数据的发行包，在验签器存在前仍不可信，也不能被未来生产运行台安装。
...
`CF-CRE@1` 只有在以下条件全部具备后，才能超出 `validation_only`：
