...
function getRemoteServiceLabel(node: FlowNode) {
  ...
  if (configuredService) {
    return configuredService.toLowerCase().includes('comfy') ? 'ComfyUI' : configuredService
  }
  const specs = getToolSpecs(node)
  if (!specs.length) return 'Remote'
  const remote = specs.find((item) => {
    const signature = JSON.stringify({
      id: item.mcp_tool_id || item.tool_id || '',
      server: item.server || '',
      tool: item.tool || '',
      params: item.params || {},
    }).toLowerCase()
    if (/comfyui|comfy_ui|comfy/.test(signature)) return true
    if (/krea|runway|pika|remote_service/.test(signature)) return true
    const urlMatches = signature.match(/https?:\/\/[^"\\\s]+/g) || []
    return urlMatches.some((url) => !/https?:\/\/(127\.0\.0\.1|localhost)(:\d+)?/i.test(url))
  })
  if (!remote) return ''
  const signature = JSON.stringify(remote).toLowerCase()
  if (signature.includes('comfy')) return 'ComfyUI'
  if (signature.includes('krea')) return 'Krea'
  return 'Remote'
}
