  ...
  return (
    <HStack justify="space-between" className="cf-page-header cf-workbench-header">
      <HStack gap={3} className="cf-workbench-titlebar">
        <Text className="cf-kicker">Flow Workbench</Text>
        <Heading size="lg" className="cf-page-title">{detail.cartridge.name}</Heading>
        <Text className="cf-page-subtitle">{detail.cartridge.id} · {detail.cartridge.category || 'flow'} · {detail.graph.mode}</Text>
      ...
      <HStack gap={2}>
        <Button className="cf-outline-btn" onClick={onBack}>返回主界面</Button>
        ...
        <Button className={mode === 'design' ? 'cf-accent-btn' : 'cf-outline-btn'} onClick={() => onModeChange('design')}>设计台</Button>
        <Button className={mode === 'run' ? 'cf-accent-btn' : 'cf-outline-btn'} onClick={() => onModeChange('run')}>测试台</Button>
      </HStack>
    </HStack>
  )
}
