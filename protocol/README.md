...
协议类别先区分 Base、Flow Authoring 与 Release Envelope，再在类别下按版本定位。例如 `flow-authoring/1.0/` 是 CF-FARP@1.0 的完整发布单元。新增协议或版本时必须新增独立目录，不得复制旧正文后继续叠加；迁移资料放在版本目录的非规范迁移模块或治理目录，不能成为当前运行语义的一部分。
