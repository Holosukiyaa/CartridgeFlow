# CartridgeFlow Base Contract v0.3

契约编号：`CARTRIDGEFLOW-BASE-0.3`
...
```json
{
  ...
  "supported_base_contracts": [
    {"id": "CARTRIDGEFLOW-BASE", "version": "0.3", "status": "current"}
  ],
  "supported_protocols": [
...
2. `supported_base_contracts` MAY 显式列出仍受支持的历史 Base Contract；这只允许旧卡带按原契约运行，不会让旧契约获得本版本的新语义。
3. `supported_protocols` 只列出本实现愿意运行的协议版本。
4. `partial` 表示只支持该协议的声明能力子集，不得包装成完整支持。
5. capability 只有在 evidence manifest 中存在实现与测试证据时才可声明。
6. 外部真实能力没有经过真实环境验证时 MUST 标记为 partial、external_unverified 或等价状态。
...
Base MAY 承载由当前 CF-FARP 发布显式列入 `trusted_subprotocols` 的内部子协议。子协议不能凭目录存在或名称约定获得信任；Base 必须同时在 `supported_subprotocols` 中声明精确 ID、版本、适配器和宿主协议范围。

受信任子协议 MUST：

1. 拥有独立完整的版本化 registry、正文、profile 和 capability 快照。
2. 只扩展宿主协议明确授权的边界，不得改写 Root Flow 的节点身份、执行计划或失败语义。
3. 在物化前校验，在未知版本、摘要不匹配或非法字段出现时失败关闭。
4. 让打包和运行记录固定引用实际消费的子协议版本与发布身份。
5. 不得保存 API key、token、私有 endpoint 或其他本机秘密。

`BASE_IMPLEMENTATION.json` v0.3 新增：
  ...
  "supported_subprotocols": [
    {
      ...
      "version": "1.0",
      "status": "supported",
      "runtime_adapter": "cf-tuning.repository.v1",
      "host_protocols": [{"id": "CF-FARP", "version": "1.1"}]
    }
  ]
}
```
...
开发态调优历史属于本机创作控制面，不得自动打入卡带。发布时 Base 必须生成只包含被激活配方快照的可移植文档，并在加载或运行前将该快照确定性地物化到 Root Flow 副本；源 `root.flow.json` 保持不变。

节点调优修订与配方发布必须不可变。回滚通过切换激活的已发布快照完成，不得覆写历史。每次运行、Artifact 和诊断必须记录配方发布 ID、Flow 摘要和节点修订集合。

## 21. v0.3 一致性增量

- [ ] 未被宿主 CF-FARP 信任的子协议在物化前被拒绝。
- [ ] 开发调优历史与可移植发布快照分离。
- [ ] 已发布配方不可变，激活与回滚保留历史。
- [ ] 运行记录固定引用实际物化的配方版本。
- [ ] 子协议内容不含本机秘密，也不能修改执行拓扑。
