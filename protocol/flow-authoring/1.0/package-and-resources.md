# CF-FARP@1.0 - Package and resources
...
9. v1.0 卡带 MUST 要求 `flow_analysis` profile，以及 `structured_io_contract`、`explicit_input_binding`、`execution_plan_runtime`、`execution_plan_static_conformance`、`execution_plan_compile`、`flow_analysis_report_v1` 和 `analysis_report_freshness_guard` 最低能力；缺失时不得以 v1.0 运行或认证。
