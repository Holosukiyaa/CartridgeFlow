...
CF-FARP v1.0 为 MCP/DLC 工具定义内部透明执行要求。目标是防止 Root Flow 退化为一个 Python 应用启动图：画布必须拥有业务编排，Python 只能实现原子操作。
...
| `legacy_opaque` | 缺少透明执行证据的兼容工具 | 可兼容运行，不能取得 v1.0 透明认证 |
