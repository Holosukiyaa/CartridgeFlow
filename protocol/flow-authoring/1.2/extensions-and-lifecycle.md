# CF-FARP@1.1 - Extensions and lifecycle
...
13. `script_policy` 在 v1.1 MUST 为 `external_hashed_only`：禁止 inline script、inline module、`eval`、`new Function`、字符串定时器、动态生成代码、WebAssembly 和未列入 files 的动态 import。
...
下列能力在 v1.1 中禁止授予 frontend component：Pending Interaction 最终提交、任意 Store 读写、任意节点执行、任意路由跳转、模型或工具直调、任意网络代理、凭据读取、Base 文件系统、主前端状态、其他卡带数据、任意 HTML 注入和永久后台任务。
