# CF-FARP@1.1 - Overview and terms
...
- Root Flow 之外、由受信任子协议管理的节点调优修订与不可变配方发布快照。
- 配方发布身份在打包、运行、事件、Artifact 与诊断中的端到端来源追踪。
...
```text
cf-farp-1-1-certified
```

规则：

1. v1.1 是完整、模块化的发布单元；规范性语义仅来自本目录的发布工件与 README 所列模块。
2. 已按其他版本认证的卡带不得使用 v1.1 标签。
3. Base 只有在 `supported_protocols` 中声明 v1.1 时才能运行该卡带。
...
| Trusted Subprotocol | 由当前 CF-FARP 发布显式授权、受 Base 校验且不能改写 Root Flow 拓扑语义的内部协议 |
| Tuning Revision | 对单个节点可调字段的一次不可变修订 |
| Recipe Release | 固定 Flow 摘要与节点调优修订集合的不可变发布快照 |
