...
CF-FARP v1.1 为 MCP/DLC 工具定义内部透明执行要求。目标是防止 Root Flow 退化为一个 Python 应用启动图：画布必须拥有业务编排，Python 只能实现原子操作。
...
兼容工具无法提供本模块规定的证据时必须标记为 `legacy_opaque`，只能兼容运行，不能取得透明执行认证。
...
声明 `CF-FARP@1.1` 的卡带必须在 `runtime_contract.required_profiles` 中包含 `tool_transparency`，并要求与其透明度等级匹配的能力。Base 只有在 `BASE_IMPLEMENTATION.json` 与 conformance evidence 同时声明时，才可以运行或认证 v1.1。
...
| `legacy_opaque` | 缺少透明执行证据的兼容工具 | 可兼容运行，不能取得 v1.1 透明认证 |

## 6. Portable DLC descriptor v3

v1.1 DLC descriptor 使用：
...
- `dlc/backend/entry.py:invoke` 这类多工具动态 dispatcher 不得通过 v1.1 透明认证。
...
- v1.1 compound tool 不得是 `legacy_opaque`。

`legacy_opaque` 可以在兼容模式运行，但不能取得 v1.1 透明认证。

## 17. dev.ai-tech-daily 迁移要求

`dev.ai-tech-daily` 不得继续用多工具 `dlc/backend/entry.py` dispatcher 取得 v1.1 认证。目标结构：
...
Workbench API surface should include:
