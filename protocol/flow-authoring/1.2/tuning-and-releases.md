# CF-FARP@1.2 - Trusted AI authoring contract

CF-FARP@1.2 trusts only CF-TUNING@1.1 under `ai_authoring_contract`.
CF-TUNING owns immutable portable recipe blueprints, fixed instances,
revision-checked AI change sets, progressive freeze snapshots, redacted source
references, and deterministic compilation references. It MUST NOT create or
alter Root Flow states, `execution_plan`, executor, effect, permissions, code,
secrets, local paths, or unsafe public values.

A freeze is valid only when each frozen step is explicitly named with its
semantic digest and instance revision. CF-FARP alone compiles accepted facts
into topology. Signed CF-FARP packages retain the compiled topology and
executable contract; their provenance pins the freeze and compilation digests
