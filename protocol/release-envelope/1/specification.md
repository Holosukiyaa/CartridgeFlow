...
本协议定义签名输入和元数据。参考实现对规范化的 `release.manifest.json` 与 `hashes.json` 执行 Ed25519 验签，并要求签名者存在于本地信任库；未受信或被篡改的包不能安装或激活。市场级密钥托管、吊销和跨设备信任同步不属于本地 Base 的最小实现。
...
`CF-CRE@1` 在参考 Base 中当前为 active/supported。以下证据已闭环并构成可安装、可激活的本地最小支持：

1. 发行构建器能生成本文要求的精确布局、规范输入、哈希和签名。
2. 密码学验证器能根据配置的本地信任库验证 Ed25519 签名。
3. 运行台 handoff 能暂存、验证、安装并激活通过兼容性检查的载荷。
4. 公开合同与载荷边界分别校验，运行台不执行包外代码。
5. 正反向一致性测试覆盖路径穿越、摘要篡改、签名失败、信任失败和运行链路。

市场接收、跨设备密钥治理、版本共存升级回滚和生产级授权仍需运行台产品单独交付，不影响本地参考 Base 的协议状态。
