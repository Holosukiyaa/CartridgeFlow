...
`--mock` 是**本地确定性分支**：模型节点不会发 HTTP 请求，而是直接使用载荷中声明的 `mock_decision_envelope`。它适合验证协议解析、执行顺序、MCP artifact 和运行结果格式。
...
demo 将非 sequence 边排除在最小执行循环之外，因此不能代表完整的 failure、branch、fork、join、loop 或 wait 语义。生产运行台必须根据 FARP@1.0 的完整执行计划规范实现这些语义，而不是复制 demo 的限制。
