...
app = FastAPI(title="CartridgeFlow", version="0.0.0-pre", default_response_class=UTF8JSONResponse)
...
@app.get("/api/health")
def health():
    return {"ok": True, "app": "CartridgeFlow", "version": "0.0.0-pre"}
...
def _schema_property_from_hint(name: str, description: str) -> dict:
    ...
    enum_map = {
        "auto/comfyui/local/off": ["auto", "comfyui", "local", "off"],
...
def import_cartridge(payload: CartridgeImportPayload):
    ...
    tmp_root = ROOT / ".data" / "tmp_imports"
    tmp_dir = tmp_root / f"import_{_uuid.uuid4().hex}"
    extract_dir = tmp_dir / "package"
    installed_root = ROOT / ".data" / "installed_cartridges"
...
def package_cartridge(cartridge_id: str, payload: CartridgePackagePayload | None = None):
    ...
    if package_mode == "production":
        ...
        if compatibility.get("legacy") or not compatibility.get("ok") or delivery_level != "production":
            raise HTTPException(status_code=400, detail={
                "error": "production_preflight_failed",
                "message": "Production package requires non-legacy manifest, no blockers, and delivery_readiness.level=production.",
    ...
    out_dir = ROOT / ".data" / "cartridge_packages"
...
def uninstall_cartridge(cartridge_id: str):
    ...
    package_path = Path(cartridge.get("package_path") or "")
    installed_root = (ROOT / ".data" / "installed_cartridges").resolve()
    try:
...
def control_cartridge_run(run_id: str, payload: CartridgeRunControl):
    try:
        return runner.control(run_id, payload.action)
    except FileNotFoundError as e:
        raise HTTPException(status_code=404, detail=str(e))
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
...
def serve_package_file(filename: str):
    package_dir = (ROOT / ".data" / "cartridge_packages").resolve()
