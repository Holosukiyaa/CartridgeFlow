...
class CartridgeRunner:
    ...
    def create_run(
        ...
        local_resource_report = resolve_cartridge_resources(manifest)
        if local_resource_report.get("status") == "blocked":
            blockers = [
                f"{item.get('role')}: {item.get('state')}"
                for item in local_resource_report.get("items") or []
        ...
        run = {
            ...
            "artifacts": [],
            "errors": [],
            "status_history": [{"from": None, "to": "created", "reason": "run_created", "at": created_at}],
            "delivery": None,
        ...
        self._write_json(run_dir / "run.json", run)
        self._append_event(run_id, cartridge_id, "run_started", root_flow.get("start") or "created", "Root Flow 开始执行", {"status": run["status"]})
        ...
        def handle_run(_state_doc: dict):
            ...
            if runtime_result.get("status") in {"failed", "error", "unavailable"}:
                failure = {"action": "runtime", "failed": True, "error_code": "DEPENDENCY_UNAVAILABLE", "error": str(runtime_result.get("error") or "Runtime adapter failed")}
                envelope = self._record_node_failure(run, run_dir, "run", failure)
                runtime_result["error_envelope"] = envelope
                _state_doc["context"]["_abort_flow"] = {"state": "run", "reason": envelope["message"], "error_id": envelope["error_id"], "action": "runtime"}
        ...
        def handle_environment(_state_doc: dict):
            ...
            if environment.get("status") == "blocked":
                failure = {"action": "environment_check", "failed": True, "error_code": "DEPENDENCY_UNAVAILABLE", "error": environment.get("summary")}
                envelope = self._record_node_failure(run, run_dir, "environment_check", failure)
                environment["error_envelope"] = envelope
                _state_doc["context"]["_abort_flow"] = {"state": "environment_check", "reason": envelope["message"], "error_id": envelope["error_id"], "action": "environment_check"}
        ...
        def handle_dependencies(_state_doc: dict):
            ...
            if dependencies.get("status") == "blocked":
                failure = {"action": "dependency_resolution", "failed": True, "error_code": "DEPENDENCY_UNAVAILABLE", "error": dependencies.get("summary")}
                envelope = self._record_node_failure(run, run_dir, "dependency_resolution", failure)
                dependencies["error_envelope"] = envelope
                _state_doc["context"]["_abort_flow"] = {"state": "dependency_resolution", "reason": envelope["message"], "error_id": envelope["error_id"], "action": "dependency_resolution"}
            ...
            def _handle(_state_doc: dict):
                ...
                try:
                    ...
                    if result.get("action") in {"tool_call", "remote_call"}:
                        ...
                        if self._node_requires_artifact(state_) and not artifacts:
                            result.update({
                                "failed": True,
                                "error_code": "ARTIFACT_MISSING",
                                "error": "Artifact-producing node completed without a valid artifact.",
                            })
                    if self._node_delivery_output_missing(state_, store):
                        result.update({
                            "failed": True,
                            "error_code": "DELIVERY_OUTPUT_MISSING",
                            "error": "Delivery node did not produce its declared primary output.",
                        })
                    if self._tool_call_was_cancelled(result):
                        ...
                        self._set_run_status(run, "paused_waiting_user", "pending_interaction")
                    ...
                    error_envelope = self._record_node_failure(run, run_dir, state_name_, result, exc)
                    if abort_on_failed:
                        _state_doc["context"]["_abort_flow"] = {
                            "state": state_name_,
                            "reason": error_envelope["message"],
        ...
        for state_name_ in root_flow_states:
            handlers.setdefault(state_name_, lambda _state_doc: None)
        handlers = self._checkpoint_handlers(handlers, root_flow_states, run, run_dir, state_doc)
    ...
    def answer_pending_interaction(
        ...
        *,
        ...
        self._set_run_status(run, "running", "pending_interaction_answered")
    ...
    def _continue_run(
        ...
        self._append_event(
            ...
            run["current_state"],
            run_event_message,
            {"status": run["status"], "data_chain": data_chain, "error_envelope": run.get("error")},
        )
        self._write_json(run_dir / "root_flow_state.json", state_doc)
        self._write_json(run_dir / "run.json", run)
        return run
    ...
    def _set_run_status(self, run: dict, target: str, reason: str) -> None:
        current = str(run.get("status") or "")
        if current == target:
            return
        transition(run, target, entity="run")
        ...
        for node_id, handler in handlers.items():
            def _wrapped(doc: dict, *, _node_id=node_id, _handler=handler):
                ...
                try:
                    ...
                    envelope = self._record_node_failure(run, run_dir, _node_id, result, exc)
                    doc.setdefault("context", {})["_abort_flow"] = {
                        "state": _node_id,
                    ...
                    self._append_event(
                        run.get("run_id") or "",
                        run.get("cartridge_id") or "",
                        "lab_node_failed",
                        _node_id,
                        f"Node {_node_id} failed: {envelope['message']}",
                        result,
                    )
                ...
                if self._run_cancelled_on_disk(run.get("run_id") or ""):
                    ...
                    if run.get("status") != "cancelled":
                        self._set_run_status(run, "cancelled", "cancel_requested")
                ...
                if context.get("_cancel_flow") and str((context.get("_cancel_flow") or {}).get("state") or "") == _node_id:
                    ...
                    outcome = "completed"
                events = self.get_events(run.get("run_id") or "")
                event_snapshot = events[-1] if events else None
                after = self.checkpoint_manager.save(
                    run_dir,
                    run,
                    doc,
                    node_id=_node_id,
                ...
                self._write_json(run_dir / "run.json", run)
                self._write_json(run_dir / "root_flow_state.json", doc)
            wrapped[node_id] = _wrapped
        return wrapped

    def _node_replay_metadata(self, state: dict, run: dict) -> dict:
        params = state.get("params") if isinstance(state.get("params"), dict) else {}
        effect = str(state.get("effect") or params.get("effect") or "none").strip() or "none"
        allowed = state.get("allowed_tools") or params.get("allowed_tools") or []
        if isinstance(allowed, str):
            allowed = [item.strip() for item in allowed.replace("\r", "\n").replace(",", "\n").split("\n") if item.strip()]
        manifest_tools = {str(item.get("id")): item for item in run.get("mcp_tools") or [] if isinstance(item, dict) and item.get("id")}
        contracts = [
            (manifest_tools.get(str(tool_id)) or {}).get("contract") or {}
            for tool_id in allowed if str(tool_id) in manifest_tools
        ]
        safe_effects = {"none", "read_only", "writes_store"}
        replay_safe = effect in safe_effects or bool(contracts and all(contract.get("idempotent") is True for contract in contracts))
        return {
            "effect": effect,
            "tool_ids": [str(item) for item in allowed],
            "idempotent": bool(contracts and all(contract.get("idempotent") is True for contract in contracts)),
            "contracts": [self._tool_contract_metadata(contract) for contract in contracts],
            "replay_safe": replay_safe,
            "requires_confirmation": not replay_safe,
            "reason": "safe_effect" if effect in safe_effects else "idempotent_tools" if replay_safe else "side_effect_not_replay_safe",
        }

    def _tool_contract_metadata(self, contract: dict) -> dict:
    ...
    def _node_requires_artifact(self, state: dict) -> bool:
        params = state.get("params") if isinstance(state.get("params"), dict) else {}
        return str(state.get("effect") or params.get("effect") or "").strip() == "writes_artifacts"

    def _run_cancelled_on_disk(self, run_id: str) -> bool:
    ...
    def _tool_call_was_cancelled(self, result: dict) -> bool:
    ...
    def _node_delivery_output_missing(self, state: dict, store: dict) -> bool:
        params = state.get("params") if isinstance(state.get("params"), dict) else {}
        kind = str(state.get("kind") or params.get("kind") or "").strip()
        if kind != "delivery":
            return False
        output = str(state.get("primary_output") or params.get("primary_output") or state.get("output") or params.get("output") or "").strip()
        if not output:
            return False
        value = store.get(output)
    ...
    def _mark_interrupted_runs(self) -> None:
        for path in self.runs_dir.glob("*/run.json"):
            try:
                ...
                continue
            if run.get("status") not in {"running", "retrying", "recovering", "rolling_back"}:
                continue
            try:
                self._set_run_status(run, "interrupted", "base_process_restarted")
            except ValueError:
                run["status"] = "interrupted"
            run["interrupted_at"] = now_iso()
            self._write_json(path, run)
            self._append_event(
                run.get("run_id") or path.parent.name,
                run.get("cartridge_id") or "",
                "run_interrupted",
                run.get("current_state") or "interrupted",
                "Run was interrupted by a base process restart.",
                {"status": "interrupted", "latest_checkpoint": run.get("latest_checkpoint")},
            )
    ...
    def control_with_options(
        ...
        *,
        ...
        if action in {"retry_current_node", "resume_checkpoint", "rollback_to_node", "restart_run"}:
    ...
    def list_checkpoints(self, run_id: str) -> list[dict]:
        self.get_run(run_id)
        return self.checkpoint_manager.list(self.runs_dir / run_id)
    ...
    def recover_run(
        ...
        *,
        ...
        checkpoints = self.checkpoint_manager.list(run_dir)
        if not checkpoints:
            raise ValueError("Run has no durable checkpoints")
        if action == "restart_run":
            unsafe = [item for item in checkpoints if item.get("phase") == "before" and (item.get("replay") or {}).get("requires_confirmation")]
            if unsafe and not confirm_side_effect:
                raise RuntimeFailure(build_runtime_error(
                    "REPLAY_CONFIRMATION_REQUIRED",
                    run_id=run_id,
                    source="runtime.recovery.restart_run",
                    context={"action": action},
                ))
            restarted = self.create_run(
                current.get("cartridge_id") or "",
                current.get("inputs") or {},
                probe_range=current.get("probe_range"),
                test_mode=current.get("test_mode") or {},
            )
            restarted["parent_run_id"] = run_id
            restarted["recovery_action"] = "restart_run"
            restarted["recovery_feedback"] = feedback or {}
            self._write_json(self.runs_dir / restarted["run_id"] / "run.json", restarted)
            self._append_event(
                run_id,
                current.get("cartridge_id") or "",
                "run_restarted",
                current.get("current_state") or "",
                "A new run was started from the original inputs.",
                {"new_run_id": restarted["run_id"], "feedback": feedback or {}},
            )
            return restarted

        if action == "retry_current_node":
            node_id = str((current.get("error") or {}).get("node_id") or current.get("current_state") or "").strip()
            ...
            if not node_id:
                raise ValueError("target_node is required for rollback_to_node")
            summary = self.checkpoint_manager.latest(run_dir, phase="before", node_id=node_id)
            recovery_status = "rolling_back"
        else:
            raise ValueError(f"Unsupported recovery action: {action}")
        if not summary:
            raise ValueError(f"No suitable checkpoint for {action}")

        replay_items = [
            item for item in checkpoints
            if item.get("phase") == "before" and int(item.get("revision") or 0) >= int(summary.get("revision") or 0)
        ]
        unsafe = [item for item in replay_items if (item.get("replay") or {}).get("requires_confirmation")]
        if unsafe and not confirm_side_effect:
            raise RuntimeFailure(build_runtime_error(
                "REPLAY_CONFIRMATION_REQUIRED",
                run_id=run_id,
                node_id=str(unsafe[0].get("node_id") or ""),
                source=f"runtime.recovery.{action}",
                context={"action": action},
            ))

        self._set_run_status(current, recovery_status, action)
        self._write_json(run_dir / "run.json", current)
        ...
        restored.pop("pending_interaction", None)
        restored["latest_checkpoint"] = summary
        previous_artifacts = current.get("artifacts") or []
        restored_ids = {str(item.get("artifact_id") or item.get("name") or "") for item in restored.get("artifacts") or [] if isinstance(item, dict)}
        invalidated = [
            {**item, "status": "invalidated", "invalidated_by": action, "invalidated_at": now_iso()}
            for item in previous_artifacts
            if isinstance(item, dict) and str(item.get("artifact_id") or item.get("name") or "") not in restored_ids
        ]
        restored["invalidated_artifacts"] = [*(current.get("invalidated_artifacts") or []), *invalidated]
        recovery_record = {
            "action": action,
            "checkpoint_id": summary["checkpoint_id"],
            "checkpoint_revision": summary.get("revision"),
            "target_node": target_node,
            "feedback": feedback or {},
            "confirmed_side_effect_replay": bool(confirm_side_effect),
            "invalidated_artifact_ids": [item.get("artifact_id") or item.get("name") for item in invalidated],
            "at": now_iso(),
        }
        current_state_doc = self._read_json(run_dir / "root_flow_state.json")
        current_context = current_state_doc.get("context") or {} if isinstance(current_state_doc, dict) else {}
        restored_context = state_doc.get("context") or {} if isinstance(state_doc, dict) else {}
        current_store = current_context.get("store") or {}
        restored_store = restored_context.get("store") or {}
        current_approvals = current.get("approvals") or current_context.get("approvals") or []
        restored_approvals = restored.get("approvals") or restored_context.get("approvals") or []
        restored_approval_ids = {
            str(item.get("approval_id") or item.get("id") or item.get("node_id") or "")
            for item in restored_approvals if isinstance(item, dict)
        }
        invalidated_approval_ids = [
            str(item.get("approval_id") or item.get("id") or item.get("node_id") or "")
            for item in current_approvals if isinstance(item, dict)
            and str(item.get("approval_id") or item.get("id") or item.get("node_id") or "") not in restored_approval_ids
        ]
        current_cache = current_context.get("cache") or current_context.get("caches") or {}
        restored_cache = restored_context.get("cache") or restored_context.get("caches") or {}
        current_error = current.get("error") if isinstance(current.get("error"), dict) else {}
        recovery_record["invalidated_store_keys"] = sorted(set(current_store) - set(restored_store))
        recovery_record["invalidated_approval_ids"] = [item for item in invalidated_approval_ids if item]
        recovery_record["invalidated_cache_keys"] = sorted(set(current_cache) - set(restored_cache)) if isinstance(current_cache, dict) and isinstance(restored_cache, dict) else []
        recovery_record["failure_experience"] = {
            "error_id": str(current_error.get("error_id") or ""),
            "code": str(current_error.get("code") or ""),
            "node_id": str(current_error.get("node_id") or ""),
            "category": str(current_error.get("category") or ""),
            "recovery_actions": list(current_error.get("recovery_actions") or []),
        }
        restored["recovery_history"] = [*(current.get("recovery_history") or []), recovery_record]
        context = state_doc.setdefault("context", {})
        context.pop("_pause_flow", None)
        context.pop("_abort_flow", None)
        context.pop("_cancel_flow", None)
        ...
        if summary.get("phase") == "after":
            ...
            if not next_states:
                ...
                self._set_run_status(restored, "completed", "checkpoint_was_terminal")
                self._write_json(run_dir / "run.json", restored)
                self._write_json(run_dir / "root_flow_state.json", state_doc)
                return restored
            start_state = next_states[0]
        else:
            history = state_doc.get("history") or []
            if history and history[-1].get("state") == start_state and history[-1].get("status") == "entered":
                history.pop()
        excluded = self._flow_descendants(root_flow, start_state, include_start=True)
        completed_parents = self._completed_parent_map_from_history(root_flow, state_doc, exclude_states=excluded)
        visited = self._completed_visited_from_history(state_doc, exclude_states=excluded)
        initial_queue = self._resume_initial_queue(root_flow, start_state, visited, completed_parents)
        self._set_run_status(restored, "running", "checkpoint_loaded")
    ...
    def _record_node_failure(
        ...
        if exception is None:
            ...
            envelope = build_runtime_error(
                ...
                run_id=run.get("run_id") or "",
                node_id=node_id,
                source=f"runtime.node.{result.get('action') or 'unknown'}",
                missing_inputs=result.get("missing_inputs") or [],
                context={"action": result.get("action")},
            )
            write_diagnostic(run_dir, envelope, exception, {"node_id": node_id, "result": result})
        result["error_envelope"] = envelope
        run["error"] = envelope
        errors = run.setdefault("errors", [])
        if not any(item.get("error_id") == envelope["error_id"] for item in errors if isinstance(item, dict)):
