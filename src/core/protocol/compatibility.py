...
from .flow_contract import build_v02_flow_contract_report, build_v03_flow_contract_report, build_v04_flow_contract_report, build_v05_flow_contract_report, build_v06_flow_contract_report, build_v07_flow_contract_report, build_v08_flow_contract_report
...
def build_compatibility_report(
    ...
    if (protocol_id, protocol_version) not in base_protocols:
        ...
        if lifecycle.get("status") == "recognized":
            suffix = f"; migrate to {target_id}@{target_version}" if target_id and target_version else ""
            findings.append(_finding("blocker", "recognized_unsupported_protocol", f"Protocol is recognized but not executable by this Base: {protocol_id}@{protocol_version}{suffix}"))
        else:
    ...
    implementation_base_contract = base.get("base_contract") if isinstance(base.get("base_contract"), dict) else {}
    required_base_id = str((base_contract or {}).get("id") or "") if isinstance(base_contract, dict) else ""
    required_base_version = str((base_contract or {}).get("version") or "") if isinstance(base_contract, dict) else ""
    ...
    if protocol_id == "CF-FARP" and protocol_version in {"0.6", "0.7", "0.8"}:
    ...
    return {
        ...
        "base_contract": {
            "required": f"{required_base_id}@{required_base_version}" if required_base_id and required_base_version else None,
            "implemented": f"{implementation_base_id}@{implementation_base_version}" if implementation_base_id and implementation_base_version else None,
            "supported": base_contract_supported,
        },
        "profiles": {
...
def _tool_pack_for_tool(tool: dict) -> str:
    ...
    if tool_type in {"local_resource", "cartridge_dlc"}:
        return ""
    if tool_type == "base_builtin":
        tool_type = "builtin"
