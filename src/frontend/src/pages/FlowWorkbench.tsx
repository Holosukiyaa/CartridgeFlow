...
export default function FlowWorkbench({ flowId, mode: controlledMode, onBack, onModeChange, onSwitchFlow }: {
  flowId: string
  mode?: WorkbenchMode
  onBack: () => void
  ...
  const [internalMode, setInternalMode] = useState<WorkbenchMode>('design')
  ...
  const pollRunUntilStable = useCallback(async (runId: string, maxAttempts = 900) => {
    ...
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      if (['completed', 'failed', 'cancelled', 'interrupted', 'paused_waiting_user'].includes(runData.status)) break
    }
    return latest
  }, [])
