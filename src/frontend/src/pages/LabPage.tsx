    ...
    <Card.Root key={flow.id} className="cf-cartridge-card cf-flow-card">
      ...
      <Card.Footer p={0} pt={0}>
        <VStack gap={1} align="stretch" className="cf-card-actions cf-flow-actions">
          <HStack gap={2} className="cf-project-card-actions">
            <Button className="cf-accent-btn" w="100%" onClick={() => navigate(projectPath(flow.id))}>设计</Button>
            <Button className="cf-outline-btn" w="100%" onClick={() => navigate(projectPath(flow.id, 'test'))}>测试</Button>
            <Button className="cf-outline-btn" w="100%" onClick={() => navigate(`/projects/${encodeURIComponent(flow.id)}/models`)}>模型配方</Button>
          </HStack>
          {flow.editable && (
    ...
    <Box className="cf-page">
      ...
      <HStack justify="space-between" className="cf-page-header">
        <VStack align="start" gap={1}>
          <Text className="cf-kicker">Flow Library</Text>
          <Heading size="lg" className="cf-page-title">Flow 管理</Heading>
          <Text className="cf-page-subtitle">设计、验证和打包专属服务卡带</Text>
        </VStack>
        <HStack gap={2}>
          <input
            ref={importPickerRef}
