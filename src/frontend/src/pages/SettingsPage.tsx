...
import { DEFAULT_APPEARANCE, loadAppearance, saveAppearance, type AppearanceSettings, type DensityMode, type FontFamilyMode, type ScrollbarMode } from '../appearance.ts'
...
export default function SettingsPage() {
  ...
  const section = searchParams.get('section') === 'environment' ? 'environment' : 'appearance'
  ...
  function selectSection(next: 'appearance' | 'environment') {
    setSearchParams(next === 'environment' ? { section: 'environment' } : {}, { replace: true })
  }

  return (
    <div className="cf-resource-page cf-settings-page">
      <header className="cf-resource-heading">
        <div><span className="cf-resource-kicker">STUDIO / SETTINGS</span><h1>系统设置</h1><p>界面偏好、本机变量与底座运行环境</p></div>
        {section === 'appearance' && <button type="button" className="cf-settings-reset" onClick={reset}>恢复默认</button>}
      </header>
      <nav className="cf-settings-tabs" aria-label="系统设置分类">
        <button type="button" className={section === 'appearance' ? 'active' : ''} onClick={() => selectSection('appearance')}>外观与可读性</button>
        <button type="button" className={section === 'environment' ? 'active' : ''} onClick={() => selectSection('environment')}>本机环境</button>
      </nav>
      {section === 'appearance' ? (
        <div className="cf-settings-layout">
          ...
          <section className="cf-settings-preview">
            ...
            <div className="cf-settings-preview-surface">
              ...
              <h2>专属服务开发工作台</h2>
              <p>模型负责生成，MCP、远程 API 与基座插件负责执行和提供外部上下文。所有敏感连接只保存在当前底座。</p>
              <div><code>Base 0.2 / CF-FARP@0.6</code><b>运行环境已准备</b></div>
              <button type="button">示例操作</button>
            </div>
            <dl className="cf-settings-facts"><div><dt>保存范围</dt><dd>当前浏览器</dd></div><div><dt>字体比例</dt><dd>{settings.fontScale}%</dd></div><div><dt>密度</dt><dd>{settings.density === 'comfortable' ? '舒展' : '紧凑'}</dd></div><div><dt>滚动条</dt><dd>{settings.scrollbarMode === 'subtle' ? '悬停显示' : '始终显示'}</dd></div><div><dt>动效</dt><dd>{settings.reducedMotion ? '已减少' : '标准'}</dd></div></dl>
          </section>
        </div>
