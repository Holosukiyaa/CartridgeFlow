...
const fitOptions = {
  padding: { top: '8%', right: '4%', bottom: '30%', left: '4%' },
  minZoom: 0.55,
  maxZoom: 1,
} as const
...
function layout(nodes: CanvasNode[], edges: Edge[], vertical: boolean) {
  ...
  nodes.forEach((node) => graph.setNode(node.id, { width: 252, height: 174 }))
  edges.forEach((edge) => graph.setEdge(edge.source, edge.target))
  dagre.layout(graph)
  return nodes.map((node) => {
    const point = graph.node(node.id)
    return { ...node, position: { x: point.x - 126, y: point.y - 87 } }
  })
}
...
export function CreatorCanvas({ creator, selectedId, onSelect }: {
  ...
  >
    ...
    <MiniMap
      pannable
      zoomable
      nodeColor={(node) => node.data.state === 'confirmed' ? '#3d9465' : node.data.state === 'unresolved' ? '#df9a36' : node.data.state === 'empty' ? '#17858a' : '#7b99ae'}
      maskColor="rgba(244, 248, 248, .68)"
      nodeStrokeWidth={3}
