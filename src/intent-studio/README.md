...
`ui-policy.json` 是机器可读的 UI 施工策略。`scripts/ui-policy-check.mjs` 使用 TypeScript AST 检查页面，不依赖类名或源码字符串凑结果。现存原生控件按文件和标签登记递减基线：可以减少，不能增加；新页面默认不允许原生 `button`、`input`、`textarea`、`select`、`dialog`、内联视觉样式或额外 CSS 文件。基线只记录迁移债务，不代表批准继续复制旧实现。

治理仓的 `check.product.intent-ui` 还独立保存同一策略的批准上限；产品不能通过修改自身策略 JSON 放宽基线、依赖白名单或 CSS 预算，降低债务时必须同步更新治理证据。
...
首次运行浏览器验收前执行 `npx playwright install chromium`。目标视口为 `1920x1080`、`1536x864`、`1120x800` 和 `390x844`；自动检查页面横向溢出、控件裁切、桌面/窄屏面板挂载、Modal 焦点恢复和画布稳定尺寸，并把截图写入仓库忽略的 `output/playwright/`。
