...
class ProtocolV04RegistryTest(unittest.TestCase):
    def test_protocol_v04_is_registered_and_claimed_partial_by_base(self):
        registry = ProtocolRegistry(ROOT)
        self.assertTrue(registry.supports_protocol("CF-FARP", "0.4"))
        ...
        self.assertIn(("CF-FARP", "0.4"), supported)
        self.assertEqual("partial", supported[("CF-FARP", "0.4")].get("status"))
